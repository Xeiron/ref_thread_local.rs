...
impl <T> RefManager<T> {
  pub fn new(local_key: &'static LocalKey<RefManagerDataGuard<T>>, init_func: fn() -> T) -> Self {
    RefManager { local_key, init_func }
  }
...
impl <T> RefThreadLocal<T> for RefManager<T> {
  fn initialize(&self) -> Result<(), ()> {
    self.local_key.with(|guard| {
      if guard.peek_data.get().ptr_inner_data.is_null() {
        ...
        guard.peek_data.set(RefManagerPeekData { ptr_inner_data, ptr_borrow_count, ptr_value });
        Ok(())
      }
      else { Err(()) }
    })
  ...
  fn is_initialized(&self) -> bool {
    self.local_key.with(|guard| {
      !guard.peek_data.get().ptr_inner_data.is_null()
    })
  }
  ...
  fn try_borrow<'a>(&self) -> Result<Ref<'a, T>, BorrowError> {
    ...
    Ok(Ref { borrow_count: cell_borrow_count, value: unsafe { ptr_value.as_ref() }.unwrap() })
  }
  ...
  fn try_borrow_mut<'a>(&self) -> Result<RefMut<'a, T>, BorrowMutError> {
    ...
    Ok(RefMut { borrow_count: cell_borrow_count, value: unsafe { ptr_value.as_mut() }.unwrap() })
  }
}
...
impl <T> RefManagerDataGuard<T>
{
  ...
  pub fn destroy(&self) -> Result<(), ()>{
    ...
    let (ptr_inner_data, ptr_borrow_count) = (peek_data.ptr_inner_data, peek_data.ptr_borrow_count);
    if ptr_inner_data.is_null() { Err(()) }
    else {
